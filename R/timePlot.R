##' Plot time series
##' 
##' Plot time series quickly, perhaps for multiple pollutants, grouped
##' or in separate panels.
##' 
##' The \code{timePlot} is the basic time series plotting function in 
##' \code{openair}. Its purpose is to make it quick and easy to plot
...
##' \emph{on the same scale}, or (if \code{group = FALSE}) each
...
##' first, which can be down be setting the \code{normalise} option.
##' 
##' The user has fine control over the choice of colours, line width
##' and line types used. This is useful for example, to emphasise a
##' particular variable with a specific line type/colour/width.
##' 
...
##' remove the key and/or the strip by setting \code{key} and/or
##' \code{strip} to \code{FALSE}. One reason to do this is to maximise
##' the plotting area and therefore the information shown.
##' 
##' @param mydata A data frame of time series. Must include a
##'   \code{date} field and at least one variable to plot.
##' @param pollutant Name of variable to plot. Two or more pollutants
##'   can be plotted, in which case a form like \code{pollutant =
##'   c("nox", "co")} should be used.
##' @param group If more than one pollutant is chosen, should they all
##'   be plotted on the same graph together? The default is
##'   \code{FALSE}, which means they are plotted in separate panels
##'   with their own scaled. If \code{TRUE} then they are plotted on
##'   the same plot with the same scale.
##' @param stack If \code{TRUE} the time series will be stacked by
...
##'   single plot.
##' @param normalise Should variables be normalised? The default is is
##'   not to normalise the data. \code{normalise} can take two values,
##'   either \dQuote{mean} or a string representing a date in UK
##'   format e.g. "1/1/1998" (in the format dd/mm/YYYY). If
##'   \code{normalise = "mean"} then each time series is divided by
...
##'   \code{group = TRUE} is often useful too to show all time series
...
##'   \dQuote{percentile} option - see below. Not used if
##'   \code{avg.time = "default"}.
##' @param percentile The percentile level in \% used when
##'   \code{statistic = "percentile"} and when aggregating the data
##'   with \code{avg.time}. More than one percentile level is allowed
##'   for \code{type = "default"} e.g. \code{percentile = c(50, 95)}.
##'   Not used if \code{avg.time = "default"}.
##' @param date.pad Should missing data be padded-out? This is useful
##'   where a data frame consists of two or more "chunks" of data with
##'   time gaps between them. By setting \code{date.pad = TRUE} the
##'   time gaps between the chunks are shown properly, rather than
##'   with a line connecting each chunk. For irregular data, set to
##'   \code{FALSE}. Note, this should not be set for \code{type} other
##'   than \code{default}.
...
##'   useful to use \code{log = TRUE} together with \code{group =
##'   TRUE}.
##' @param windflow This option allows a scatter plot to show the wind
##'   speed/direction as an arrow. The option is a list e.g.
##'   \code{windflow = list(col = "grey", lwd = 2, scale = 0.1)}. This
##'   option requires wind speed (\code{ws}) and wind direction
##'   (\code{wd}) to be available.
##'   
##'   The maximum length of the arrow plotted is a fraction of the
##'   plot dimension with the longest arrow being \code{scale} of the
...
##'   angle. The list may contain other options to \code{panel.arrows}
##'   in the \code{lattice} package. Other useful options include 
...
##'   ensure over-plotting does not become a problem.
##' @param smooth Should a smooth line be applied to the data? The
##'   default is \code{FALSE}.
##' @param ci If a smooth fit line is applied, then \code{ci}
##'   determines whether the 95\% confidence intervals are shown.
...
##'   Can include \dQuote{top}, \dQuote{bottom}, \dQuote{right} and
##'   \dQuote{left}.
...
##'   \code{layout} for panel arrangement and \code{pch} and
##'   \code{cex} for plot symbol type and size and \code{lty} and
##'   \code{lwd} for line type and width, as passed to \code{xyplot},
##'   although some maybe locally managed by \code{openair} on route,
##'   e.g. axis and title labelling options (such as \code{xlab}, 
##'   \code{ylab}, \code{main}) are passed via \code{quickText} to
##'   handle routine formatting. See examples below.
##' @export
##' @return As well as generating the plot itself, \code{timePlot}
...
##'   retained, e.g. using \code{output <- timePlot(mydata, "nox")},
...
##'   \code{\link{timeAverage}} for details on selecting averaging
